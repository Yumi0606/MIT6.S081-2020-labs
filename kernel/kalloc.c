...
struct ref_stru {
  struct spinlock lock;
  int cnt[PHYSTOP / PGSIZE];  // 每个物理页一个计数
} ref;
...
freerange(void *pa_start, void *pa_end)
{
    ...
    for (; p + PGSIZE <= (char*)pa_end; p += PGSIZE) {
        ref.cnt[(uint64)p / PGSIZE] = 1; // 初始化为1
        kfree(p);
    }
}
...
int cowpage(pagetable_t pagetable, uint64 va) {
  ...
  return (*pte & PTE_F) ? 0 : -1; // 返回 0 表示是 COW 页
}
...
cowalloc(pagetable_t pagetable, uint64 va)
{
    ...
    if (krefcnt((void*)pa) == 1) {
        ...
        *pte &= ~PTE_V; // 先取消映射，避免 remap 冲突
        ...
        kfree((void*)pa); // 减少旧页引用，kfree 内部会判断是否为0
        return mem;
    }
}