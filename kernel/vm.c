...
// 从内核页表取消映射用户地址
void
uvmunmapinkpt(pagetable_t kernelpt, uint64 oldsz, uint64 newsz)
{
  for(uint64 va = PGROUNDUP(newsz); va < oldsz; va += PGSIZE){
    pte_t *pte = walk(kernelpt, va, 0);
    if(pte == 0)
      continue;
    uint64 pa = PTE2PA(*pte);
    kfree((void*)pa);
